...
def main() -> int:
    p = argparse.ArgumentParser(description="Query artifacts index.jsonl across outputs/")
    ...
    p.add_argument("--kind", default=None, help="Filter by artifact kind (exact match)")
    p.add_argument("--contains", default=None, help="Substring filter over JSON text")
    p.add_argument("--limit", type=int, default=50, help="Max records to print (default: 50)")
    args = p.parse_args()
